    ...
    <div class="board-area">
      ...
      >
        ...
        <div
          ...
          :key="
            item.getCoordinate()[0] + '_' + item.getCoordinate()[1]
          "
          class="tile"
    ...
    <div class="panel">
      ...
      <div class="stats">
        ...
        <div class="stat-row">
          ...
          <span class="stat-value">{{ fmtTime(elapsed) }}</span>
        </div>
        <div v-if="bestTime" class="stat-row best">
          <span class="stat-label">最佳</span>
          <span class="stat-value">{{ fmtTime(bestTime) }} / {{ bestSteps }}步</span>
        </div>
      </div>

      <div class="actions">
        <el-button type="primary" @click="handleStart" :disabled="playing">
          {{ over ? '再来一局' : '开始' }}
        </el-button>
      ...
      <div v-if="over" class="win-banner">
        🎉 拼图完成！<br/>
        {{ fmtTime(elapsed) }} · {{ count }}步
      </div>
    </div>
  </div>
</template>
...
import { exampleImg, PageBgImgList } from "@/components/Options.js";
...
function getTileIndex(item: Block): number {
  ...
  const idx = (cy / itemSize.value) * tier.value + (cx / itemSize.value) + 1;
  return idx;
}
...
function init() {
  img.value =
    typeof getBgIndex === "number" ? PageBgImgList[getBgIndex] : exampleImg;
  createPuzzle();
}
...
function puzzleClick(item: Block) {
  if (item.getIsImg()) {
