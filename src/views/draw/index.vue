<template>
  <div ref="drawEl" id="draw" @keydown="onKeyDown" tabindex="0">
    ...
    <div class="toolbar">
      ...
      <div class="tool-group">
        <el-color-picker v-model="color" :predefine="predefineColors" @change="onColorChange" :disabled="isEraser" size="small" />
      </div>
      <div class="tool-group width-group">
        <div
          v-for="w in widthList" :key="w"
          class="width-dot" :class="{ active: lineWidth === w }"
      ...
      <div class="tool-group brush-group">
        <el-button v-for="(name, index) in brushNames" :key="index" size="small"
      ...
      <div class="tool-group">
        <el-button size="small" :type="isEraser ? 'warning' : ''" @click="toggleEraser" title="橡皮擦">
          <el-icon><EditPen v-if="!isEraser" /><Brush v-else /></el-icon>
        </el-button>
      </div>
      <div class="tool-divider" />
      <div class="tool-group">
        <el-button :disabled="drawStep < 0" circle size="small" @click="undo" title="撤销 Ctrl+Z"><el-icon><Back /></el-icon></el-button>
        <el-button :disabled="drawStep >= drawHistory.length - 1" circle size="small" @click="redo" title="重做 Ctrl+Y"><el-icon><Right /></el-icon></el-button>
      </div>
      <div class="tool-group">
        <el-button circle size="small" @click="clearCanvas" title="清空画布"><el-icon><Delete /></el-icon></el-button>
      </div>
      <div class="tool-group">
        <el-button circle size="small" @click="saveImage" title="保存为图片 Ctrl+S"><el-icon><Download /></el-icon></el-button>
      </div>
    </div>
  </div>
</template>
...
import { Back, Right, View, Hide, ArrowLeft, Delete, Download, EditPen, Brush } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { PageBgImgList, BgSizeList } from '@/config/images'
...
const brushTypes = [
  { name: 'round', cap: 'round' as CanvasLineCap, join: 'round' as CanvasLineJoin },
  { name: 'square', cap: 'square' as CanvasLineCap, join: 'bevel' as CanvasLineJoin },
  { name: 'butt', cap: 'butt' as CanvasLineCap, join: 'miter' as CanvasLineJoin },
]
...
function onColorChange() {
  if (!isEraser.value) getCtx().strokeStyle = color.value
}
...
function redrawAll() {
  ...
  for (const stroke of visible) {
    ...
    if (stroke.isEraser) {
      ...
      ctx.lineCap = brush.cap; ctx.lineJoin = brush.join
...
function undo() { if (drawStep.value >= 0) { drawStep.value--; redrawAll() } }
function redo() { if (drawStep.value < drawHistory.value.length - 1) { drawStep.value++ } }

function clearCanvas() {
  drawHistory.value = []
  drawStep.value = -1
  currentPoints.value = []
  getCtx().clearRect(0, 0, canvasRef.value!.width, canvasRef.value!.height)
}
...
function saveImage() {
  ...
  if (showBgImg.value) {
    ...
    if (bgImg && bgImg.complete) ectx.drawImage(bgImg, 0, 0, exportCanvas.width, exportCanvas.height)
  }
...
onMounted(() => { initCanvas(); drawEl.value?.focus(); window.addEventListener('resize', onResize) })
onUnmounted(() => {
  document.removeEventListener('mousemove', onMouseMove)
  document.removeEventListener('mouseup', onMouseUp)
...
.width-group .width-dot {
  ...
  &:hover span { transform: scale(1.3); }
}

.brush-group :deep(.el-button) { font-size: 12px; padding: 3px 8px; }

:deep(.el-color-picker__trigger) { border: 1px solid rgba(255, 255, 255, 0.3); }
</style>
